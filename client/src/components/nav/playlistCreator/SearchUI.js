...
} from '../../../styles/playlistCreator';

const SearchUI = ({ songList, setSongList, isOpenPlaylistCreator }) => {
  // 서치 바 ref
  ...
  const handleVideoClick = (song) => {
    const isAddable = songList.every(
    ...
    if (isAddable) {
      ...
      setSongList((prev) => prev.concat(songData_toBeSavedInBackend));
    } else {
  ...
  // 추가된 노래를 songList에서 삭제
  // 전제: songList에는 같은 곡을 추가할 수 없다 <- handleVideoClick(song)에서 이미 있는 곡은 추가할 수 없음
  ...
  return (
    <SearchUIContainer>
      ...
      {/* 추가된 곡들 정보 */}
      <div className='addedSongsInfo'>{`총 ${songList.length}곡`}</div>
      {/* 추가된 곡들 리스트 */}
      <div className='addedSongs'>
        ...
        {/* 플레이리스트가 비어있을 때 */}
        {songList.length === 0 && <p className='empty-playlist'>텅</p>}
      </div>
      {/* 유튜브 검색 결과 */}
